...
/**
 * Platform rules — cooldown windows, attempt limits, the assignment timer.
 *
 * These aren't cosmetic: `applyOutcome` reads them, so this row is a control,
 * not a preferences blob. A missing row yields `new VaultSettings()` — the same
 * defaults the client fell back to.
 */
...
export async function saveSettings(s: VaultSettings, cx?: PoolConnection): Promise<void> {
  ...
  await db.query(
    ...
    [
      ...
      s.assignmentExpiryDays, s.portfolioStaleDays, s.officeIp,
